                ...
                if (conn.getResponseCode() == 200) {
                    ...
                    if (json.length() > 5) {
                        ...
                        runOnUiThread(() -> {
                            ...
                            // Account-Liste nicht leer -> Link wurde (zwischenzeitlich,
                            // z.B. im Lobby) abgeschlossen: Code-Anzeige entsorgen
                            if (prefs.getString("link_code", null) != null) {
                                prefs.edit().remove("link_code").apply();
                                if (tvStatus != null) {
                                    tvStatus.setText(R.string.link_status_linked);
                                    tvStatus.setTextColor(0xFF00E676);
                                    tvStatus.setVisibility(View.VISIBLE);
                                }
                                if (tvInstructions != null) tvInstructions.setVisibility(View.GONE);
                            }
    ...
    private void fetchLinkStatus(String appUuid, String expectedCode) {
        new Thread(() -> {
            try {
                ...
                for (int i=0; i<30; i++) {
                    Thread.sleep(2000);
                    // Seit dem Security-Fix: Profil-Poll ueber die Token-RPC statt Tabellen-SELECT
                    java.net.URL url = new java.net.URL(eu.kodanetwork.mchost.security.PraetorSecurity.getSupabaseUrl() + "/rest/v1/rpc/rpc_get_user_profile");
                    java.net.HttpURLConnection conn = (java.net.HttpURLConnection) url.openConnection();
                    conn.setRequestMethod("POST");
                    conn.setDoOutput(true);
                    conn.setRequestProperty("Content-Type", "application/json");
                    ...
                    if (respCode == 200) {
                        ...
                        // Verlinkt, wenn der Code serverseitig konsumiert wurde oder
                        // ein mc_username eingetragen ist
                        boolean codeConsumed = !expectedCode.equals(obj.optString("code", ""));
                        boolean hasMc = obj.has("mc_username") && !obj.isNull("mc_username");
                        if (codeConsumed || hasMc) {
                            prefs.edit().remove("link_code").apply();
                            runOnUiThread(() -> {
