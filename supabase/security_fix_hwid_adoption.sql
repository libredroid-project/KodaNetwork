...
NOTIFY pgrst, 'reload schema';
-- Link zweistufig: Plugin erstellt PENDING-Anfrage, App bestaetigt.
...
BEGIN
  ...
  IF v_count > 0 THEN
    RETURN 'ok';
  END IF;
  -- Sonst: Anfrage merken (ueberschreibt alte Anfrage)
  UPDATE public.koda_users
...
-- App: ausstehende Link-Anfrage abrufen
...
-- App: Anfrage bestätigen oder ablehnen
